...
# Build the TF-M BL1/BL2 secure image set and the tf-m-tests non-secure app for
# AN521, using the local uv-managed Python environment and the manually
# installed Arm GNU 14.2.Rel1 toolchain, then start the result in QEMU from
# BL1_1.
#
# This script is the BL1-on-AN521 bring-up variant of
# .scripts/build_mps2_an521_spe_app.sh.
...
echo
echo Config and Build SPE with BL1/BL2
echo
...
qemu-system-arm \
    ...
    -monitor none \
    -serial stdio \
    -kernel build/api_ns/bin/bl1_1.axf \
    -device loader,file=build/api_ns/bin/bl1_2_padded.bin,addr=0x10010000 \
    -device loader,file=build/api_ns/bin/bl2_signed.bin,addr=0x10020000 \
    -device loader,file=build_test/tfm_s_signed.bin,addr=0x100a0000 \
    -device loader,file=build_test/tfm_ns_signed.bin,addr=0x10120000
